...
static uint64 prepare_heap(pgtbl_t new_pgtbl, inode_t* ip, elf_header_t* eh) {
	...
	for (uint32 off = eh->ph_off; off < eh->ph_off + eh->ph_ent_num * sizeof(ph); off += sizeof(ph)) {
		...
		// 用户堆生长
		int pte_flags = PTE_U;
		if (ph.flags & ELF_PROG_FLAG_READ)  pte_flags |= PTE_R;
		if (ph.flags & ELF_PROG_FLAG_WRITE) pte_flags |= PTE_W;
		if (ph.flags & ELF_PROG_FLAG_EXEC)  pte_flags |= PTE_X;
		new_heap_top = uvm_heap_grow(new_pgtbl, old_heap_top,
			ph.va + ph.mem_size - old_heap_top, pte_flags);
...
int proc_exec(char* path, char** argv) {
	...
	assert(trapframe != NULL, "proc_exec: trapframe alloc failed");
	pgtbl_t pgtbl = proc_pgtbl_init((uint64)trapframe);
	...
	elf_header_t eh;
	if (inode_read_data(ip, 0, sizeof(elf_header_t), &eh, false) != sizeof(elf_header_t) || eh.magic != ELF_MAGIC) {
		inode_unlock(ip);
		inode_put(ip);
		uvm_destroy_pgtbl(pgtbl);
		return -1;
	...
	uint64 new_heap_top = prepare_heap(pgtbl, ip, &eh);
	if (new_heap_top == -1) {
		inode_unlock(ip);
		inode_put(ip);
		uvm_destroy_pgtbl(pgtbl);
	...
	int argc;
	uint64 sp = prepare_stack(pgtbl, argv, &argc);
	if (sp == -1) {
		uvm_destroy_pgtbl(pgtbl);
		return -1;
	}
	...
	p->tf->user_to_kern_sp = p->kstack + 2 * PGSIZE;
	...
	p->ustack_npage = 1;
	memmove(p->name, path, sizeof(p->name));
