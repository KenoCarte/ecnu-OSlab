...
proc_t* proc_alloc() {
    for (int i = 0;i < N_PROC;i++) {
        ...
        if (p->state == UNUSED) {
            ...
            assert(trapframe != NULL, "proc_alloc: trapframe alloc failed");
            p->pgtbl = proc_pgtbl_init((uint64)trapframe);
            p->tf = (trapframe_t*)trapframe;
...
void proc_make_first() {
    ...
    proczero->tf->user_to_kern_sp = proczero->kstack + 2 * PGSIZE;
    ...
    while (1) {
        ...
        for (int r = 0;r < N_PROC;r++) {
            ...
            if (p->state == RUNNABLE) {
                ...
                swtch(&c->ctx, &p->ctx);
                // printf("cpu %d: scheduled pid %d (state now %d)\n", mycpuid(), p->pid, p->state);
                c->proc = NULL;
                last = i;
            }
            spinlock_release(&p->lk);
