...
static uint32 bitmap_search_and_set(uint32 bitmap_block_num, uint32 valid_count) {
    ...
    for (uint32 byte = 0; byte < BLOCK_SIZE && valid_count>0; byte++) {
        if (buf->data[byte] != 0xFF) {
            for (uint32 shift = 0; shift < BIT_PER_BYTE && valid_count>0; shift++) {
            ...
            uint32 skip = valid_count < BIT_PER_BYTE ? valid_count : BIT_PER_BYTE;
            valid_count -= skip;
        }
    }
    buffer_put(buf);
...
uint32 bitmap_alloc_block() {
    for (uint32 block = sb.data_bitmap_firstblock; block < sb.data_bitmap_firstblock + sb.data_bitmap_blocks; block++) {
        uint32 valid_count = BIT_PER_BLOCK;
        if (block == sb.data_bitmap_firstblock + sb.data_bitmap_blocks - 1) {
            valid_count = sb.data_blocks % BIT_PER_BLOCK;
            if (valid_count == 0)
                valid_count = BIT_PER_BLOCK;
        }
        uint32 index = bitmap_search_and_set(block, valid_count);
        if (index != -1) {
            return (block - sb.data_bitmap_firstblock) * BIT_PER_BLOCK + index + sb.data_firstblock;
        }
    }
    return -1;
}
...
uint32 bitmap_alloc_inode() {
    for (uint32 block = sb.inode_bitmap_firstblock; block < sb.inode_bitmap_firstblock + sb.inode_bitmap_blocks; block++) {
        uint32 valid_count = BIT_PER_BLOCK;
        if (block == sb.inode_bitmap_firstblock + sb.inode_bitmap_blocks - 1) {
            valid_count = sb.total_inodes % BIT_PER_BLOCK;
            if (valid_count == 0)
                valid_count = BIT_PER_BLOCK;
        }
        uint32 index = bitmap_search_and_set(block, valid_count);
        if (index != -1) {
            return (block - sb.inode_bitmap_firstblock) * BIT_PER_BLOCK + index;
        }
    }
    return -1;
}
...
void bitmap_free_block(uint32 block_num) {
    uint32 bitmap_block_num = sb.data_bitmap_firstblock + (block_num - sb.data_firstblock) / BIT_PER_BLOCK;
    uint32 index = (block_num - sb.data_firstblock) % BIT_PER_BLOCK;
    bitmap_clear(bitmap_block_num, index);
}
